...
class Predict(object):
    ...
    def __init__(self, f, pids, yids, Df=None, p0=None, rank=None, ptype='', 
                 rdeltap=None, pred=None, **kwargs):
        """

        :param f: takes np.array as input and output, stored as attribute _f
        :param Df: takes np.array as input and output, stored as attribute _Df
        """
        if pred is not None:
            f = pred._f
            Df = pred._Df
        ...
        def f(p=None):
            """Broaden the call signature to allow for p as list/tuple/np.array/pd.Series
            """
            if p is None:
                p = p0
            y = _f(np.array(p))
